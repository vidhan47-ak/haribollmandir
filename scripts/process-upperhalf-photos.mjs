...
const inDir = "D:/haribolll/_gdrive_upper_files";
...
async function processUpperHalfPhotos() {
  if (!fs.existsSync(inDir)) {
    console.log("No _gdrive_upper_files folder found.");
    return;
  }

  const files = fs.readdirSync(inDir).filter((f) => /\.(jpg|jpeg|png|webp)$/i.test(f));
  console.log(`Processing ${files.length} upper-half photos...`);

  let counter = 1;
  for (const f of files) {
    const inPath = path.join(inDir, f);
    const outName = `upper-photo-${counter}.webp`;
...
processUpperHalfPhotos().catch((err) => console.error("Error processing upper-half photos:", err));
